...
const Recent = () => {
  ...
  const fetchPosts = async (isInitialLoad = false) => {
    ...
    setIsLoading(true);
    try {
        const results = await fetch(`http://192.168.1.98:3000/api/v1/memoryBasedCollaborativeFilteringRecent`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ user_uuid: userId.user_uuid, limit, offset })
        })
        ...
        const postsWithState = data.data.map((post: any) => ({
            ...post,
