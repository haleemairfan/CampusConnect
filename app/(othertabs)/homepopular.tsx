...
const Popular = () => {
  ...
  const fetchPopularPosts = async (isInitialLoad = false) => {
    if (isLoading || (!isInitialLoad && !hasMore)) return;

    setIsLoading(true);
    try {
      const results = await fetch(`http://192.168.1.98:3000/api/v1/getPopularPosts/${userId.user_uuid}?limit=${limit}&offset=${offset}`, {
        method: 'GET',
        headers: {
  ...
  const renderPost = ({ item }: { item: Post }) => {
    ...
    let userFlairs = item.users.user_flairs
      ? item.users.user_flairs
        .replace(/^\{|\}$/g, '')
        .split(',')
        .map(flair => flair.replace(/"/g, '').trim())
      : [];
